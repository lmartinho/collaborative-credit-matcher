...
def optimize(problem, parameters):
    ...
    for solution in problem.getSolutionIter():
        utility = evaluate(utility_function, parameters, solution)
        score = utility["score"]
        if score > best_score:
        ...
        if time.time() > time_limit:
            ...
            matcher_utils.print_solution(parameters, best_solution)
...
def calculate_tightness(parameters, results):
    # calculate the overall member rate margin
    lender_rates_margins = calculate_lender_rates_margins(parameters, results)
    borrower_rates_margins = calculate_borrower_rates_margins(parameters, results)
...
def calculate_lender_rates_margins(parameters, results):
    lender_parameters = parameters["lenders"]
    lender_results = results["lenders"]
...
def calculate_borrower_rates_margins(parameters, results):
    borrower_parameters = parameters["borrowers"]
    borrower_results = results["borrowers"]
...
#                                 "6": {"maximum_rate" : 0.01, "minimum_amount" : 30, "maximum_amount" : 300}}}
    ...
    solution, score = solve(parameters)
